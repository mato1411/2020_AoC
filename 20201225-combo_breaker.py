...
def get_loop_size(pub_key):
    ...
    loop_size = 0
    calculated_pub_key = start_val

    # for number of times (= loop size) do this to calc the public
    while calculated_pub_key != pub_key:
        loop_size += 1
        calculated_pub_key *= subject_number
        calculated_pub_key %= divider
...
if __name__ == '__main__':
    ...
    files = ['20201225-example.txt', '20201225-input.txt']
    ...
    for file_name in files:
        ...
        for pub_key in lines:
            loop_size = get_loop_size(int(pub_key))
            devices.append({'pk': int(pub_key), 'ls': loop_size})

        ek1 = get_encryption_key(devices[0].get('pk'), devices[1].get('ls'))
        ek2 = get_encryption_key(devices[1].get('pk'), devices[0].get('ls'))
